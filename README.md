...
- **表示モード切り替え**：左右分割ペイン / インライン（変更前 → 変更後 を 1 行内に表示）
...
- **接続設定のエクスポート / インポート**（JSON ファイルでチーム間共有が可能）
...
| ボタン | 動作 |
|--------|------|
| **エクスポート** | 登録済みの接続設定を JSON ファイルとしてダウンロード（パスワードは含まれません） |
| **インポート** | エクスポートした JSON ファイルを読み込んで接続設定を一括追加（同名の設定はスキップ） |

> パスワードはセキュリティ上エクスポートされません。インポート後に各接続設定を編集してパスワードを入力してください。
